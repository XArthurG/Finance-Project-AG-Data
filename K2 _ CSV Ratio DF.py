...
for i in range(length):
    ...
    url_0 = 'Alphabet Inc (GOOG)/' + input_date_0
    url_1 = 'Alphabet Inc (GOOG)/' + input_date_1
    url_2 = 'Alphabet Inc (GOOG)/' + input_date_2
    
    date_input = input_date_0[:-6]
    ...
    file = pd.read_csv(url_0)
    file = file.drop(file.columns[2:], axis=1)
    file = file.set_axis([date_input + '-0', 'Default'], axis=1, inplace=False)


    col_one_list = file[date_input + '-0'].tolist()
    ...
    col_one_arr = file.loc[file[date_input + '-0'].isin(['Marketable securities',
    ...
    file = pd.read_csv(url_1)
    file = file.drop(file.columns[2:], axis=1)
    file = file.set_axis([date_input + '-1', 'Default'], axis=1, inplace=False)
    file = file.dropna()

    col_one_list = file[date_input + '-1'].tolist()
    #print(f"\ncol_one_list:\n{col_one_list}\ntype:{type(col_one_list)}")
    ...
    col_one_arr = file.loc[file[date_input + '-1'].isin(['Revenues',
                                          'Net income',
    ...
    file = pd.read_csv(url_2)
    file = file.drop(file.columns[2:], axis=1)
    file = file.set_axis([date_input + '-2', 'Default'], axis=1, inplace=False)

    col_one_list_0 = file[date_input + '-2'].tolist()
    #print(f"\ncol_one_list:\n{col_one_list}\ntype:{type(col_one_list)}")
    ...
    col_one_arr = file.loc[file[date_input + '-2'].isin(['Net income',
                                          'Depreciation',
                                          'Amortization',
                                          'Prepaid expenses',
    ...
    col_one_arr = file.loc[file[date_input + '-0'].isin(['Retained earnings'])].to_numpy()
    ...
    file = file.set_axis([date_input + '-1', 'Default'], axis=1, inplace=False)
    col_one_arr = file.loc[file[date_input + '-1'].isin(['Net income'])].to_numpy()
    ...
    col_one_arr = file.loc[file[date_input + '-0'].isin(["Current assets"])].to_numpy()
    Current_assets_start = float(col_one_arr[0,1])
    col_one_arr = file.loc[file[date_input + '-0'].isin(["Current liabilities"])].to_numpy()
    Current_liabilities_start = float(col_one_arr[0,1])
    col_one_arr = file.loc[file[date_input + '-0'].isin(["Property, plant and equipment"])].to_numpy()
    ...
    file = file.set_axis([date_input + '-2', 'Default'], axis=1, inplace=False)    
    col_one_list = file[date_input + '-2'].tolist()
    ...
    col_one_arr = file.loc[file[date_input + '-2'].isin(["Depreciation"])].to_numpy()
    ...
    filename = 'Alphabet Inc (GOOG)/' + input_date_0[:-6] + "-Ratio-DF.csv"
    df.to_csv(filename)
